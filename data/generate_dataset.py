...
def generate_h5py_instance_csv(
    path_raw_data,
    path_h5py,
    point_no=8000,
    split_ratio={"tr": 0.6, "va": 0.2, "te": 0.2},
    ...
    for i, f in enumerate(files):
        pcd = load_smlm_ply_with_labels(os.path.join(path_raw_data, f))
        if pcd is None:
            print(f">>> Error occured. Ignoring {f}.")
            continue
    ...
    with h5py.File(
        os.path.join(path_h5py, "smlm_instance_segmentation_train.hdf5"), "w"
    ) as f:
        for k in train_dataset.keys():
            f.create_dataset(k, data=train_dataset[k])

    with h5py.File(
        os.path.join(path_h5py, "smlm_instance_segmentation_validation.hdf5"), "w"
    ) as f:
        for k in validation_dataset.keys():
            f.create_dataset(k, data=validation_dataset[k])

    with h5py.File(
        os.path.join(path_h5py, "smlm_instance_segmentation_test.hdf5"), "w"
    ) as f:
        for k in test_dataset.keys():
            f.create_dataset(k, data=test_dataset[k])
...
def main():
    ...
    generate_h5py_instance_csv('D:/DevPython/PlantSegNet/datasets/npcs_small', 'D:/DevPython/PlantSegNet/datasets/npc_formatted')
